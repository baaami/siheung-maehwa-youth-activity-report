...
import { RepositoryConflictError, updateReportAsAdmin } from "@/lib/repository";
...
export async function PUT(
  ...
  try {
    ...
    const report = await updateReportAsAdmin({
      clubId,
      reportId,
      input,
      status,
    ...
    return NextResponse.json({
      redirectUrl: `/admin/clubs/${clubId}/reports/${report.id}`,
    });
  } catch (error) {
